...
impl Quest {
    ...
    pub fn create(
        ...
        let spec = spec.into();
        let preimage = Self::preimage(&title, &spec, &tasks, &sigils, deadline_ts, created_ts);
        Quest {
...
mod tests {
    ...
    fn quest_roundtrips_and_verifies() {
        let a = id();
        let q = Quest::create(&a, "sum a big range", "distribute the sum", tasks(), vec!["compute".into()], 9999, 1000);
        assert!(q.verify());
    ...
    fn quest_tamper_breaks_verification() {
        let a = id();
        let mut q = Quest::create(&a, "t", "s", tasks(), vec![], 1, 1);
        q.tasks[0].spec = "shard 0..999999".into(); // change the work after signing
        assert!(!q.verify());
        let mut r = Quest::create(&a, "t", "s", tasks(), vec![], 1, 1);
        r.deadline_ts = 0;
        assert!(!r.verify());
    }
